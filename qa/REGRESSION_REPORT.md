...
## Transcript Studio implementation addendum

Date: 2026-09-15. The affected groups were the TypeScript prompt/settings/helper/tool contracts; the new Swift transcript Core; the SwiftUI editor; the native CLI protocol; packaging scripts; and the manual QA catalog. The text-only preprocessor remained the first return and the native library is initialized only by the app or an audio/library helper action.

The final TypeScript build and all 42 selected plugin selectors passed in 0.854 seconds. The included 10,000-call text-only measurement was 7.07 ms, launched no helper, and performed no configuration, history, model, or filesystem work. The repository-local Swift verification package ran 9 selected Core tests in 0.034 seconds: decoding/range validation, subtitles, editing/fillers/compact/range replacement, SQLite WAL/FTS/audio deduplication, corrupt-database recovery, rich `.tst` round trip, hostile/corrupt archive rejection with no partial record, custom templates, and quoted CSV round trip. The SwiftUI app and protocol-v3 CLI source sets both passed standalone compiler type checking against the pinned local dependency builds. There were no failed selector retries.

A new full Xcode build receipt was required because Swift sources, `project.yml`, dependencies, and the generated project changed. Five direct, project-local Xcode attempts failed before compilation during SwiftPM initialization with error 74 under the host sandbox, so no receipt exists and no `test-without-building` run was used. Standalone legacy `xcresulttool` extraction succeeded for all five preserved bundles; separate JSON processing reports one `Test` action per bundle, each `failedToStart`, with zero executed selectors. Those attempts are infrastructure history rather than test coverage. The focused Swift package verification used `--disable-sandbox` while retaining its package cache, scratch directory, module cache, fixtures, and database files under `.build-artifacts/`.

Automated accounting for the completed focused groups is 51 expected unique selectors, 51 passed, 0 failed, 0 missing, 0 unexpected, and 0 unrun. Full Xcode regression accounting remains 0 executed because the required build did not start. The QA catalog validates 78 cases and records 0 product passes; 55 selected release cases remain unpassed. Real model quality, diarization assets and DER, the 100,000-segment FTS measurement, packaged helper, actual LM Studio composer, and native UI cases remain release gates.

Failed result bundles remain preserved because the full milestone did not pass. Reusable Derived Data is about 2.1 GiB, SourcePackages about 559 MiB, and the transcript-studio verification area about 790 MiB. Each reusable cache is below 10 GiB; no scoped test artifacts were deleted.
