...
test("resolves directive, history, UI, global, and built-in precedence", () => {
  ...
  const history = { modelID: "whisper-medium-q5", language: "es", includeFilename: false, scope: "chat" as const };
  ...
  assert.deepEqual(
    ...
    { modelID: "whisper-small-q5", language: "fr", includeFilename: true, scope: "inherit" }
  );
  assert.deepEqual(
    resolveSettings({ directive: null, history: null, ui: inheritedUI, global }),
    { modelID: "whisper-tiny-multilingual", language: "de", includeFilename: false, scope: "inherit" }
  );
  assert.deepEqual(
...
test("writes, merges, reloads, and resets global settings atomically", async () => {
  ...
  assert.deepEqual(await loadGlobalSettings(environment), { version: 1 });
});
